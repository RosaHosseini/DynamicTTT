...
public class DiscriminatorTrie<I, O> {
    private final DiscriminatorTrieNode<I, O> root;
    ...
    public void insert(Word<I> word) {
        DiscriminatorTrieNode<I, O> current = root;
    ...
    private List<Word<I>> findAllCandidateDiscriminators(DiscriminatorTrieNode<I,O> node) {
        List<Word<I>> result = new ArrayList<>();
        if (node.children.size() < alphabet.size())
            for (I symbol : alphabet) {
                if (node.children.containsKey(symbol)) {
                    DiscriminatorTrieNode<I, O> child = node.children.get(symbol);
